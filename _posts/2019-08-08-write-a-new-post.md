...
The last modified date of a post is obtained according to its latest git commit date, and all the modified date of the posts should be stored in `_data/lastmod.yml`. For example:

```yaml
-
  index: 1      # the index of Jekyll site.posts
  filename: 2019-08-09-getting-started      # post filename without 
  lastmod: 2020-04-13 00:38:56 +0800        # the post last modified date
-
  # another lastmod entry
  ...
```
...
When some posts have been modified since their published date and also the file `_data/lastmod.yml` was created correctly, a list with the label **Recent Updates** will be displayed in the right panel of the desktop view, which records the five most recently modified articles.
